...
function requireSafeStableStringify() {
  return hasRequiredSafeStableStringify || (hasRequiredSafeStableStringify = 1, function(module2, exports2) {
    const { hasOwnProperty } = Object.prototype, stringify = configure();
    stringify.configure = configure, stringify.stringify = stringify, stringify.default = stringify, exports2.stringify = stringify, exports2.configure = configure, module2.exports = stringify;
...
function requireIterator() {
  ...
  return typeof Symbol < "u" && (Iterator.prototype[Symbol.iterator] = function() {
    ...
    return new Iterator(function() {
      return i >= l ? { done: !0 } : { done: !1, value: args[i++] };
    });
    ...
    return new Iterator(function() {
      return i >= l ? { done: !0 } : { done: !1, value: sequence[i++] };
    });
...
function requireForeach() {
  ...
  return foreach = function(iterable, callback) {
    ...
    if (SYMBOL_SUPPORT && Symbol.iterator in iterable && typeof iterable.next != "function" && (iterable = iterable[Symbol.iterator]()), typeof iterable.next == "function") {
      for (iterator2 = iterable, i = 0; s = iterator2.next(), s.done !== !0; )
...
function requireTypedArrays() {
  return hasRequiredTypedArrays || (hasRequiredTypedArrays = 1, function(exports2) {
    ...
    exports2.getMinimalRepresentation = function(array, getter) {
      ...
      for (i = 0, l = array.length; i < l; i++)
        v = getter ? getter(array[i]) : array[i], t = exports2.getNumberType(v), p = TYPE_PRIORITY[t.name], p > maxPriority && (maxPriority = p, maxType = t);
      return maxType;
      ...
      for (var PointerArray = exports2.getPointerArray(length), array = new PointerArray(length), i = 0; i < length; i++)
        array[i] = i;
      return array;
    };
...
function requireLruCache() {
  ...
  return LRUCache.prototype.clear = function() {
    ...
    return typeof pointer < "u" ? (this.splayOnTop(pointer), oldValue = this.V[pointer], this.V[pointer] = value, { evicted: !1, key, value: oldValue }) : (this.size < this.capacity ? pointer = this.size++ : (pointer = this.tail, this.tail = this.backward[pointer], oldValue = this.V[pointer], oldKey = this.K[pointer], delete this.items[oldKey]), this.items[key] = pointer, this.K[pointer] = key, this.V[pointer] = value, this.forward[pointer] = this.head, this.backward[this.head] = pointer, this.head = pointer, oldKey ? { evicted: !0, key: oldKey, value: oldValue } : null);
...
function requireMemory() {
  ...
  class StorageMemory extends StorageInterface {
     ...
     */
    _removeKey(key) {
      return this.log.debug({ msg: "acd/storage/memory._removeKey", key }), this.store.has(key) ? (this.store.set(key, void 0), !0) : !1;
    }
    /**
     * @param {string[]} keys
     */
...
const runtime = typeof document > "u" ? "server" : "browser", defineEnableLiveMode = (config) => {
  ...
  return (options) => {
    ...
    return Promise.resolve().then(function() {
      return require("./enableLiveMode.cjs");
    }).then(({ enableLiveMode }) => {
  ...
  function createDefaultFetcher() {
    ...
    return unstable__cache.instance = createDefaultCache(client), {
      ...
      fetch: (query, params, $fetch, controller) => {
        ...
        $fetch.setKey("loading", !0), $fetch.setKey("error", void 0), unstable__cache.instance.fetch(JSON.stringify({ query, params })).then((response) => {
